...
<script runat="server" language="C#">
    ...
    public void Page_Load(object sender, EventArgs e)
    {
        ...
        {           
            using (var client = new ProxyWebClient())
            {
                ...
                if (hasData && !DoNotFormatBody.Checked && client.ResponseHeaders != null)
                {
                    data = FormatBody(data, client.ResponseHeaders.Get("Content-Type"));
                }
    ...
    private static string FormatBody(string data, string format)
    {
        if (format.IndexOf("application/json", StringComparison.OrdinalIgnoreCase) >= 0)
        {
            return ResourceOutputFormatter.FormatJson(data);
        }

        if (format.IndexOf("application/xml", StringComparison.OrdinalIgnoreCase) >= 0 ||
            format.IndexOf("text/xml", StringComparison.OrdinalIgnoreCase) >= 0)
        {
    ...
    {
        if (headerCollection == null || headerCollection.Count == 0) return String.Empty;
...
<script type="text/javascript">
    ...
    var headerValues = {
        "accept": ["application/json", "application/json; charset=utf-8", "application/xml", "application/xml; charset=utf-8", "text/xml", "text/xml; charset=utf-8", "text/html", "text/html; charset=utf-8", "application/x-www-form-urlencoded", "application/x-www-form-urlencoded; charset=utf-8", "*/*"],
        "accept-charset": ["utf-8", "*"],
        "accept-encoding": ["gzip", "deflate", "*"],
        "accept-language": ["en-US", "es-ES", "*"],
        "content-type": ["application/json", "application/json; charset=utf-8", "application/xml", "application/xml; charset=utf-8", "text/xml", "text/xml; charset=utf-8", "text/html", "text/html; charset=utf-8", "application/x-www-form-urlencoded", "application/x-www-form-urlencoded; charset=utf-8"]
    };

    function modifyMethodState() {
        if ($("#HttpMethod").val() == "GET" || $("#HttpMethod").val() == "DELETE" || $("#HttpMethod").val() == "HEAD" || $("#HttpMethod").val() == "OPTIONS") {
            $("#BodySection").hide();
        } else {
            $("#BodySection").show();
        }
    ...
    function getResponseHeight() {
        var height = parseInt($.cookie("responseHeight"));

        if (!height || isNaN(height)) {
            height = parseInt($("#ResponseText").css("height"));
        }
    ...
    function setHeaderState() {
        var displayHeaders = $.cookie("displayHeaders");

        if (displayHeaders == "true") {
            $("#DisplayResponseHeaders").prop("checked", true);
        } else {
            $("#DisplayResponseHeaders").removeProp("checked");
        }
    }

    function setBodyFormatState() {
        var doNotFormat = $.cookie("doNotFormatBody");

        if (doNotFormat == "true") {
            $("#DoNotFormatBody").prop("checked", true);
        } else {
            $("#DoNotFormatBody").removeProp("checked");
        }
    }

    function setDecreaseHeightVisibility(height) {
        if (height <= 200) {
            $("#DecreaseResponseHeight").hide();
        } else {
            $("#DecreaseResponseHeight").show();
        }
    }
    ...
    $(function () {
        ...
        var height = getResponseHeight();

        $("#ResponseText").css("height", height);
        setDecreaseHeightVisibility(height);
    ...
    $(function () {
        $("#Reload").click(function () {
            if ($.trim($("#OperationUrl").val()) != "") {
                var redirectUrl = "proxy";

                var oid = $("#OperationId").val();
                redirectUrl += "?oid=" + encodeURIComponent(oid);

                var format = $("#ResourceFormat").val().toLowerCase();
                redirectUrl += "&ct=" + encodeURIComponent(format);
    ...
    $(function () {
        $("#DisplayResponseHeaders").change(function () {
            $.cookie("displayHeaders", $(this).is(":checked").toString(), { path: "/" });
        });

        $("#DoNotFormatBody").change(function () {
            $.cookie("doNotFormatBody", $(this).is(":checked").toString(), { path: "/" });
        });
    });

    $(function () {
        $("#IncreaseResponseHeight").click(function () {
            var height = getResponseHeight();
            height = height + 100;

            $("#ResponseText").css("height", height);
            setDecreaseHeightVisibility(height);
            $.cookie("responseHeight", height, { path: "/" });

            return false;
        });

        $("#DecreaseResponseHeight").click(function () {
            var height = getResponseHeight();
    ...
    $(function () {
        $("#ViewResponse").click(function () {
            ...
            var responseForm = ['<form id="ResponseForm" method="POST" action="output" target="_blank">'];
            responseForm.push('<input type="hidden" name="format" value="' + $("#ResourceFormat").val() + '"/>');
            responseForm.push('<input type="hidden" name="responseText" value="' + encodeURIComponent($("#ResponseText").val()) + '"/>');
            responseForm.push('</form>');

            $(responseForm.join('')).appendTo('body')[0].submit();
            return false;
        });
    });
        ...
        $(document).keydown(function (event) {
            if (!event.ctrlKey && !event.altKey && !event.shiftKey) {
                ...
                if (event.keyCode == 107) {
                    $("#IncreaseResponseHeight").click();
                    return false;
                } else if (event.keyCode == 109) {
                    $("#DecreaseResponseHeight").click();
                    return false;
                } else if (event.keyCode == 65) {
                    if ($("#DisplayResponseHeaders").is(':checked')) {
                        $("#DisplayResponseHeaders").removeProp("checked");
                    } else {
                        $("#DisplayResponseHeaders").prop("checked", true);
                    }
                    $.cookie("displayHeaders", $("#DisplayResponseHeaders").prop("checked").toString(), { path: "/" });
                    return false;
                } else if (event.keyCode == 66) {
                    $("#ViewOutput").click();
                    return false;
                } else if (event.keyCode == 68) {
                    if ($("#DoNotFormatBody").is(':checked')) {
                        $("#DoNotFormatBody").removeProp("checked");
                    } else {
                        $("#DoNotFormatBody").prop("checked", "checked");
                    }
                    $.cookie("doNotFormatBody", $("#DoNotFormatBody").prop("checked").toString(), { path: "/" });
                    return false;
                }
            }
    ...
    $(function () {
        $("#HeaderText").autocomplete({
            wordCount: 1,
            on: {
                query: function (text, currentLine, callback) {
                    var resultArray = new Array();
                    var resultIndex = 0;

                    var searchArray = null;
                    var headerSeparatorIndex = currentLine.indexOf(":");

                    if (headerSeparatorIndex < 0) {
                        searchArray = allHeaders;
                    } else if (headerSeparatorIndex > 0) {
                        var headerValue = $.trim(currentLine.substr(0, headerSeparatorIndex));
                        var pastHeaderValue = $.trim(currentLine.substr(headerSeparatorIndex - 1, currentLine.length));

                        searchArray = headerValues[headerValue.toLowerCase()];
                    ...
                    for (var i = 0; i < searchArray.length; i++) {
                        var searchValue = searchArray[i].toLowerCase();
                        var resultValue = $.trim(text).toLowerCase();

                        if (searchValue.indexOf(resultValue) == 0 && (resultValue.length == 1 || searchValue != resultValue)) {
                            resultArray[resultIndex++] = searchArray[i];
                        }
                    }
